...
use crate::idp::sp_extractor::{SPMetadataExtractor, RequiredAttribute};
...
fn test_self_signed_authn_request() {
    ...
    let _ = unverified.try_verify_self_signed().expect("failed to verify self signed signature");
}
...
fn test_extract_sp() {
    ...
    let x509cert = extractor.verification_cert().expect("failed to get x509 cert");

    let authn_request_xml = include_str!("../../test_vectors/authn_request.xml");
    verify_signed_xml(authn_request_xml, x509cert.as_slice(), Some("ID")).expect("failed to verify authn request");
...
fn test_signed_response() {
    ...
    let authn_request_xml = include_str!("../../test_vectors/authn_request.xml");
    let unverified = UnverifiedAuthnRequest::from_xml(authn_request_xml).expect("failed to parse");
    let verified = unverified.try_verify_self_signed().expect("failed to verify self signed signature");

    // create some attributes:
    let attrs = vec![("urn:oasis:names:tc:SAML:2.0:attrname-format:uri", "firstName", ""),
                    ("urn:oasis:names:tc:SAML:2.0:attrname-format:uri", "lastName", ""),
                    ("urn:oasis:names:tc:SAML:2.0:attrname-format:uri", "firstName", "")];
...
fn test_signed_response_threads() {
    ...
    handles.into_iter().for_each(|h|h.join().expect("failed thread"));
}
